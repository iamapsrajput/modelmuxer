...
def test_providers_endpoint_contract():
    ...
    for endpoint in ["/providers", "/v1/providers"]:
        ...
        assert response.status_code in [200, 404, 503], f"Unexpected status code {response.status_code} for {endpoint}"
        ...
        if response.status_code == 200:
            ...
            for provider_name, provider_info in data["providers"].items():
                ...
                assert provider_info["name"] == provider_name, f"Provider name mismatch for {endpoint}"
                assert isinstance(provider_info["models"], list), f"Models should be list for {endpoint}"
                assert provider_info["status"] in ["available", "unavailable"], f"Invalid status for {endpoint}"
...
def test_models_endpoint_contract():
    ...
    if response.status_code == 200:
        ...
        for model in data["data"]:
            ...
            assert ":" not in model["model"] and "/" not in model["model"], (
