...
class HeuristicRouter:
    ...
    def __init__(self):
        ...
        self.programming_keywords = [
            ...
            'async', 'await', 'promise', 'callback', 'lambda', 'yield',
            'SELECT', 'INSERT', 'UPDATE', 'DELETE', 'CREATE', 'DROP', 'ALTER',
            'html', 'css', 'javascript', 'python', 'java', 'cpp', 'sql', 'json'
        ]
        
        # Complexity indicators
        self.complexity_keywords = [
            'analyze', 'analysis', 'explain', 'explanation', 'debug', 'debugging',
            'reasoning', 'reason', 'complex', 'complicated', 'algorithm', 'algorithms',
            'optimize', 'optimization', 'architecture', 'design pattern', 'patterns',
            'performance', 'scalability', 'trade-off', 'tradeoffs', 'comparison',
            'evaluate', 'assessment', 'review', 'critique', 'detailed', 'comprehensive',
            'step-by-step', 'methodology', 'approach', 'strategy', 'framework',
            'implementation', 'solution', 'problem-solving', 'troubleshoot'
        ]
        
        # Simple query indicators
        self.simple_indicators = [
            'what is', 'who is', 'when is', 'where is', 'how much', 'how many',
            'define', 'definition', 'meaning', 'translate', 'translation',
            'calculate', 'convert', 'list', 'name', 'tell me', 'show me'
        ]
    ...
    def analyze_prompt(self, messages: List[ChatMessage]) -> Dict[str, any]:
        ...
        analysis['is_simple'] = (
            analysis['simple_confidence'] > 0.2 and 
            analysis['total_length'] < settings.simple_query_max_length and
            analysis['message_count'] <= 2
        )
    ...
    def select_model(
        ...
        if budget_constraint:
            # This is a simplified budget filtering - in a real implementation,
            # you'd calculate estimated costs for each model
            if budget_constraint < 0.001:  # Very low budget
                preferences = [p for p in preferences if 'mistral' in p[0] or 'haiku' in p[1]]
            elif budget_constraint < 0.01:  # Low budget
                preferences = [p for p in preferences if 'gpt-4o' not in p[1]]
