            ...
            for provider, model, estimate, usd_value in affordable_preferences:
                ...
                if adapter is not None and not getattr(adapter, "circuit_open", False):
                    ...
                    logger.debug(
                        ...
                        model,
                        (
                            original_preferences[0] != (provider, model)
                    ...
                    logger.debug(
                        "Down-routing metric: checking condition - len(original_preferences)=%d, original_preferences[0]=%s, selected=(%s, %s)",
                        len(original_preferences),
                        original_preferences[0] if original_preferences else "None",
                        provider,
                        model,
                    )
                    logger.debug(
                        "Down-routing metric: condition result = %s",
                        len(original_preferences) > 0
                        and original_preferences[0] != (provider, model),
                    )
                    if len(original_preferences) > 0 and original_preferences[0] != (
                        ...
                        logger.debug(
                            "Down-routing metric: original_preferences[0] = %s, (provider, model) = (%s, %s)",
                            original_preferences[0],
                            provider,
                            model,
                        )
                        ...
                        try:
                            ...
                            logger.debug(
                                ...
                                usd_value,
                                original_first_estimate.usd is not None
                                and usd_value < float(original_first_estimate.usd),
                            )
                            logger.debug(
                                "Down-routing metric: about to check if we should call the metric"
                            )
                            logger.debug(
                                "Down-routing metric: checking if original_first_estimate.usd is not None: %s",
                                original_first_estimate.usd is not None,
                            )
                            logger.debug(
                                "Down-routing metric: checking if usd_value < float(original_first_estimate.usd): %s < %s = %s",
                                usd_value,
                                str(float(original_first_estimate.usd)),
                                usd_value < float(original_first_estimate.usd),
                            )
                            logger.debug("Down-routing metric: about to check the final condition")
                            logger.debug(
                                "Down-routing metric: final condition result: %s",
                                original_first_estimate.usd is not None
                                and usd_value < float(original_first_estimate.usd),
                            )
                            logger.debug("Down-routing metric: about to call the metric")
                            ...
                            logger.debug(
                                "Down-routing metric: error estimating original cost: %s", e
                            )
                    if span is not None:
                        ...
                        try:
                            ...
                            if original_first_estimate.usd is not None and usd_value < float(
                                ...
                                try:
                                    ...
                                    logger.error(
                                        "Error calling down-routing metric: %s", metric_error
                                    )
                                    import traceback
