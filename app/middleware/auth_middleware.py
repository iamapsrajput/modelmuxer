...
class AuthMiddleware:
    ...
    async def _check_rate_limits(self, user_info: Dict[str, Any]) -> None:
        ...
        user_limits["minute_requests"] = {
            k: v for k, v in user_limits["minute_requests"].items()
            if k >= current_minute - 1
        }
        user_limits["hour_requests"] = {
            k: v for k, v in user_limits["hour_requests"].items()
            if k >= current_hour - 1
        }
        user_limits["day_requests"] = {
            k: v for k, v in user_limits["day_requests"].items()
            if k >= current_day - 1
        }
    ...
    def get_rate_limit_stats(self) -> Dict[str, Any]:
        ...
        active_users = sum(
            1 for user_data in self.rate_limit_storage.values()
