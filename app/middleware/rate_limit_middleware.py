...
class RateLimitMiddleware:
    ...
    async def _check_global_limits(self, current_time: float) -> None:
        ...
        if self.global_counters["current_second"]["count"] >= self.global_limits.get("requests_per_second", float('inf')):
            raise HTTPException(
                status_code=429,
                detail={
                    "error": {
        ...
        if self.global_counters["current_minute"]["count"] >= self.global_limits.get("requests_per_minute", float('inf')):
            raise HTTPException(
                status_code=429,
                detail={
                    "error": {
