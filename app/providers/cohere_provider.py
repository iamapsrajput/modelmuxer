        ...
        try:
            ...
            if e.response.status_code == 429:
                ...
                raise AuthenticationError(
                    f"Cohere API authentication failed",
                    provider=self.provider_name
                )
            else:
                error_detail = ""
    ...
    async def stream_chat_completion(
        ...
        **kwargs
        ...
        try:
            ...
            raise ProviderError(f"Cohere streaming request failed: {str(e)}", provider=self.provider_name)
        except Exception as e:
            if isinstance(e, ProviderError):
                raise
            raise ProviderError(f"Cohere streaming unexpected error: {str(e)}", provider=self.provider_name)
