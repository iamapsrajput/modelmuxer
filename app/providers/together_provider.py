        ...
        try:
            ...
            if e.response.status_code == 429:
                raise RateLimitError(
                    f"Together AI API rate limit exceeded",
                    provider=self.provider_name
                )
            elif e.response.status_code == 401:
                raise AuthenticationError(
                    f"Together AI API authentication failed",
                    provider=self.provider_name
                )
            else:
                error_detail = ""
            ...
            raise ProviderError(f"Together AI request failed: {str(e)}", provider=self.provider_name)
        except Exception as e:
            if isinstance(e, ProviderError):
                raise
            raise ProviderError(f"Together AI unexpected error: {str(e)}", provider=self.provider_name)
    ...
    async def stream_chat_completion(
        ...
        **kwargs
        ...
        try:
            ...
            raise ProviderError(f"Together AI streaming request failed: {str(e)}", provider=self.provider_name)
        except Exception as e:
            if isinstance(e, ProviderError):
                raise
            raise ProviderError(f"Together AI streaming unexpected error: {str(e)}", provider=self.provider_name)
