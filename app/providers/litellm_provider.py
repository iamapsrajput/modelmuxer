        ...
        try:
            ...
            if e.response.status_code == 429:
                raise RateLimitError(
                    f"LiteLLM proxy rate limit exceeded",
                    provider=self.provider_name
                )
            elif e.response.status_code == 401:
                raise AuthenticationError(
                    f"LiteLLM proxy authentication failed",
                    provider=self.provider_name
                )
            else:
                error_detail = ""
            ...
            raise ProviderError(f"LiteLLM proxy request failed: {str(e)}", provider=self.provider_name)
        except Exception as e:
            if isinstance(e, ProviderError):
                raise
            raise ProviderError(f"LiteLLM proxy unexpected error: {str(e)}", provider=self.provider_name)
    ...
    async def stream_chat_completion(
        ...
        **kwargs
        ...
        try:
            ...
            raise ProviderError(f"LiteLLM proxy streaming request failed: {str(e)}", provider=self.provider_name)
        except Exception as e:
            if isinstance(e, ProviderError):
                raise
            raise ProviderError(f"LiteLLM proxy streaming unexpected error: {str(e)}", provider=self.provider_name)
