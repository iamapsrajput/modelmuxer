...
class PIIProtector:
    ...
    def protect_text(self, text: str, user_id: Optional[str] = None) -> Tuple[str, List[PIIDetection]]:
        ...
        for detection in reversed(detections):
            ...
            if policy.action == RedactionAction.BLOCK:
                logger.warning("pii_request_blocked", 
                             user_id=user_id, 
    ...
    def _apply_redaction(self, detection: PIIDetection, policy: RedactionPolicy) -> str:
        ...
        if policy.action == RedactionAction.REDACT:
            ...
            encrypted_id = hashlib.md5(encrypted).hexdigest()[:8]
