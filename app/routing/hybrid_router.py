...
class HybridRouter(BaseRouter):
    ...
    async def _route_request(
        ...
        if self.heuristic_router:
            try:
                heuristic_rec = await self.heuristic_router._route_request(
                    messages, analysis.get("strategy_analyses", {}).get("heuristic", analysis), user_id, constraints
                )
        ...
        if self.semantic_router:
            try:
                semantic_rec = await self.semantic_router._route_request(
                    messages, analysis.get("strategy_analyses", {}).get("semantic", analysis), user_id, constraints
                )
        ...
        if self.cascade_router:
            try:
                cascade_rec = await self.cascade_router._route_request(
                    messages, analysis.get("strategy_analyses", {}).get("cascade", analysis), user_id, constraints
                )
        ...
        if not strategy_recommendations:
            return "openai", "gpt-3.5-turbo", "Hybrid routing fallback - no strategies available", 0.5
