...
class TestHeuristicRouter(unittest.TestCase):
    ...
    def test_code_detection(self):
        ...
        messages = [ChatMessage(role="user", content="Here's some code:\n```python\ndef hello():\n    print('Hello')\n```")]
        ...
        messages = [ChatMessage(role="user", content="Use the `print()` function in Python")]
        analysis = self.router.analyze_prompt(messages)
        self.assertTrue(analysis['has_code'])
        
        # Test without code
        messages = [ChatMessage(role="user", content="What is the weather like today?")]
        analysis = self.router.analyze_prompt(messages)
    ...
    def test_simple_query_detection(self):
        ...
        messages = [ChatMessage(role="user", content="Explain in detail the differences between various sorting algorithms, their time complexities, space complexities, and when to use each one in different scenarios.")]
        analysis = self.router.analyze_prompt(messages)
        self.assertFalse(analysis['is_simple'])
    
    def test_model_selection_code(self):
        """Test model selection for code-related prompts."""
        messages = [ChatMessage(role="user", content="Write a Python function to implement binary search")]
    ...
    def test_model_selection_simple(self):
        ...
        messages = [ChatMessage(role="user", content="What is 5+3?")]
        provider, model, reason = self.router.select_model(messages)
    ...
    def test_model_selection_complex(self):
        ...
        messages = [ChatMessage(role="user", content="Analyze the performance characteristics of different database indexing strategies")]
        provider, model, reason = self.router.select_model(messages)
    ...
    def test_multi_message_analysis(self):
        ...
        messages = [
            ChatMessage(role="user", content="I need help with programming"),
...
class TestCostTracker(unittest.TestCase):
    ...
    def test_cost_estimation(self):
        ...
        messages = [ChatMessage(role="user", content="Write a simple Python function")]
